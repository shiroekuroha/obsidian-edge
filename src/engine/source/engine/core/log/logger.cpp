...
Logger::init ()
{
    ...
    s_coreFileLogger = spdlog::rotating_logger_mt ("Engine File", "logs/engine_file.txt",
                                                   1024 * 1024 * OE_LOG_FILE_MAX_SIZE, OE_LOG_FILE_MAX_NUMBER);
    s_coreFileLogger->set_level (spdlog::level::trace);

    s_clientLogger = spdlog::stdout_color_mt ("Application");
    s_clientLogger->set_level (spdlog::level::trace);

    s_clientFileLogger = spdlog::rotating_logger_mt ("Client File", "logs/client_file.txt",
                                                     1024 * 1024 * OE_LOG_FILE_MAX_SIZE, OE_LOG_FILE_MAX_NUMBER);
