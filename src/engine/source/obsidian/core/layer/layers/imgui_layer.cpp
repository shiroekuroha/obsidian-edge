...
void
ImGuiLayer::onUpdate (float delta)
{
    ...
    ImGuiIO &io = ImGui::GetIO ();
    io.DisplaySize
        = ImVec2 ((float)Application::get ().window ().getWidth (), (float)Application::get ().window ().getHeight ());
...
void
ImGuiLayer::onEvent (SharedPointer<Event> event)
{
    ImGuiIO &io = ImGui::GetIO ();
...
auto
glfwKeyToImGuiKey (int keycode, int scancode) -> ImGuiKey
{
    IM_UNUSED (scancode);
    switch (keycode)
        {
